...
/* ---------- توزيع الأسئلة بحيث ما يتكرر نفس النوع بالتتابع (قد الإمكان) ---------- */
function diversifyByType(items){
  const buckets = {};
  items.forEach(it => {
    buckets[it.type] = buckets[it.type] || [];
    buckets[it.type].push(it);
  });
  const result = [];
  let lastType = null;
  while(result.length < items.length){
    const typeKeys = Object.keys(buckets).filter(t => buckets[t].length > 0);
    typeKeys.sort((a, b) => buckets[b].length - buckets[a].length);
    let chosen = typeKeys.find(t => t !== lastType) || typeKeys[0];
...
function renderPlacementTest(containerId, stages, opts){
  ...
  function runStage(idx){
    ...
    const items = diversifyByType(shuffle(stage.pool).slice(0, sampleSize)).map(q => prepQuestion(q));
...
function buildQuestionCard(q, idx, onAnswered){
  ...
  const tagMap = {
    ...
    match:["Vocabulary — Match","vocab"], reading:["Reading","mcq"], underline:["Find the Mistake","tf"]
  };
...
function renderFinalLevelResult(container, data, opts){
  ...
  card.innerHTML = `
    ...
    <div class="cta-row" style="justify-content:center; display:flex; gap:14px; flex-wrap:wrap; margin-top:10px;">
      <a class="btn btn-outline-dark" href="../index.html">رجوع للرئيسية</a>
    </div>
  `;

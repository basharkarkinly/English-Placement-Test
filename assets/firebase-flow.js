...
function renderSurvey(area, isRegistered, resultData, guestName){
  ...
  function renderQuestion(){
    ...
    if(q.type === "choice" || q.type === "choice_with_text"){
      q.options.forEach((opt) => {
        ...
        el.addEventListener("click", () => {
          answers[q.key] = opt;
          if(q.type === "choice"){
            setTimeout(() => { qIndex++; renderQuestion(); }, 250);
          } else {
      ...
      if(q.type === "choice_with_text") showExtraTextField();
    }

    if(q.type === "text"){
      optWrap.innerHTML = `
        <input type="text" id="survey-text-input" placeholder="اكتب جوابك هون...">
        <div class="cta-row" style="justify-content:center; display:flex; gap:10px; margin-top:12px;">
          <button class="btn btn-primary" id="survey-next-btn">التالي</button>
        </div>
      `;
    ...
    function showExtraTextField(){
      if(document.getElementById("survey-extra-text")) return;
      const wrap = document.createElement("div");
      wrap.style.marginTop = "12px";
      wrap.innerHTML = `
        <input type="text" id="survey-extra-text" placeholder="${q.textLabel}">
        <div class="cta-row" style="justify-content:center; display:flex; gap:10px; margin-top:10px;">
          <button class="btn btn-primary" id="survey-next-btn2">التالي</button>
        </div>
      `;
      qbox.appendChild(wrap);
      qbox.querySelector("#survey-next-btn2").addEventListener("click", () => {
